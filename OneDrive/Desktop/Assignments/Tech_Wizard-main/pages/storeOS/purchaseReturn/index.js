...
const PurchaseReturn = () => {
    ...
    const handleBarcodeScan = async (barcodeValue) => {
        ...
        if (existingItem) {
            setToastMessage('Item already scanned!')
            return
        }
    ...
    const handleStockUpdate = async (item) => {
        try {
            const finalStock = item.existingStock - item.decreaseQty
            ...
            const updatedItems = await Promise.all(
                scannedItems.map(async (item) => {
                    ...
                    return {
                        ...
                        decreaseQty: 0, // Reset decreaseQty
                        updateStatus: 'failed',
                    }
                })
            )
    ...
    const saveAllChanges = async () => {
        ...
        const successfulUpdates = results.filter((res) => res.success)
        ...
        if (failedUpdates.length > 0) {
            ...
            // Update scannedItems with failed items only
            setScannedItems((prevItems) =>
                prevItems.map((item) => {
                    ...
                    return item // Keep successful items as is without resetting 'updateStatus'
                })
            )
                                                                            ...
                                                                            ? {
                                                                                  ...
                                                                                  decreaseQty:
                                                                                      Math.min(
                                                                                          ...
                                                                                          i.decreaseQty +
                                                                                              1
                                                                                      ), // Increment decreaseQty but ensure it doesn't exceed existingStock
                                                                              }
                                                                            : i
                                                                )
                                                        )
